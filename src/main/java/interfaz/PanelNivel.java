...
public class PanelNivel extends JPanel {
	...
	@Override public void paintComponent ( Graphics g ) {
		...
		g.drawImage( imagen.getImage(), space.getJugadorActual().getPosInicialX(), space.getJugadorActual().getPosInicialY(), imagen.getIconWidth(), imagen.getIconHeight(), null );
		...
		for ( int i = 0; i < partida.getEnemigos().length; i++ ) {
			for ( int j = 0; j < partida.getEnemigos()[i].length; j++ ) {
				if ( partida.getEnemigos()[i][j] != null ) {
					...
					g.drawImage( icono.getImage(), partida.getEnemigos()[i][j].getPosX(), partida.getEnemigos()[i][j].getPosY(), null );
				}
			}
		}
